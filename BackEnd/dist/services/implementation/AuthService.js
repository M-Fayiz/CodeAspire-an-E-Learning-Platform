...
class AuthService {
    ...
    async signUp(user) {
        ...
        if (isUserExist) {
            throw (0, http_error_1.createHttpError)(http_status_1.HttpStatus.CONFLICT, error_message_1.HttpResponse.USER_EXIST);
        }
        ...
        const key = `${redisKey_1.redisPrefix.VERIFY_EMAIL}:${token}`;
    ...
    async verifyEmail(data) {
        const key = `${redisKey_1.redisPrefix.VERIFY_EMAIL}:${data.token}`;
        const result = await redis_config_1.default.get(key);
        if (!result) {
            throw (0, http_error_1.createHttpError)(http_status_1.HttpStatus.UNAUTHORIZED, error_message_1.HttpResponse.USER_CREATION_FAILED);
        }
        ...
        if (!newUser) {
            throw (0, http_error_1.createHttpError)(http_status_1.HttpStatus.CONFLICT, error_message_1.HttpResponse.USER_CREATION_FAILED);
        }
    ...
    async refreshAccessToken(token) {
        ...
        if (!decode) {
            throw (0, http_error_1.createHttpError)(http_status_1.HttpStatus.NOT_FOUND, error_message_1.HttpResponse.REFRESH_TOKEN_EXPIRED);
        }
        const user = await this._userRepo.findUserByEmail(decode.email);
        if (!user?.isActive) {
            throw (0, http_error_1.createHttpError)(http_status_1.HttpStatus.FORBIDDEN, error_message_1.HttpResponse.USER_BLOCKED);
    ...
    async login(email, password) {
        ...
        if (!isMatch) {
            throw (0, http_error_1.createHttpError)(http_status_1.HttpStatus.FORBIDDEN, error_message_1.HttpResponse.INVALID_CREDNTIALS);
        }
    ...
    async forgotPassword(email) {
        ...
        const key = `${redisKey_1.redisPrefix.FORGOT_PASSWORD}:${email}`;
    ...
    async resetPassword(email, token, password) {
        ...
        if (!storedToken) {
            throw (0, http_error_1.createHttpError)(http_status_1.HttpStatus.BAD_REQUEST, error_message_1.HttpResponse.TOKEN_NOT_FOUND);
        }
        if (storedToken !== token) {
            throw (0, http_error_1.createHttpError)(http_status_1.HttpStatus.FORBIDDEN, error_message_1.HttpResponse.UNAUTHORIZED);
        }
