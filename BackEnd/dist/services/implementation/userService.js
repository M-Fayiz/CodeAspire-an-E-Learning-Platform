...
class UserService {
    ...
    async changePassword(id, currentPassword, newPassword) {
        ...
        if (!passwordIsMatch) {
            throw (0, http_error_1.createHttpError)(http_status_1.HttpStatus.NOT_FOUND, error_message_1.HttpResponse.INVALID_CREDNTIALS);
        }
    ...
    async updateUserProfile(id, userData) {
        ...
        if (!handler) {
            throw (0, http_error_1.createHttpError)(http_status_1.HttpStatus.BAD_REQUEST, `Invalid user role: ${user.role}`);
        }
        return handler();
    }
    ...
    async addMentorData(mentorId, mentorData) {
        ...
        const updateMentorData = await this._mentorRepository.updateMentorProfile(mentor_Id, { ...mentorData, ApprovalStatus: "requested" });
        if (!updateMentorData) {
