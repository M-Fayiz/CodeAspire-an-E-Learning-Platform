...
const intitializeSocket = (server) => {
    ...
    io.use((socket, next) => {
        ...
        try {
            ...
            next(new Error(error_message_1.HttpResponse.UNAUTHORIZED));
        }
    });
    io.on(socketEvents_1.SocketEvents.CONNECT, async (socket) => {
        ...
        socket.on(socketEvents_1.SocketEvents.DISCONNECT, async () => {
            await redis_config_1.default.hDel(redisKey_1.redisPrefix.OFFLINE_USERS ?? redisKey_1.redisPrefix.ONLINE_USERS, userId);
            await redis_config_1.default.hDel(redisKey_1.redisPrefix.ONLINE_USERS, userId);
            io.emit(socketEvents_1.SocketEvents.USER_OFFLINE, userId);
