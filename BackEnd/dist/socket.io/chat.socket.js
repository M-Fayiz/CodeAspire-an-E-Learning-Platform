...
const socketEvents_1 = require("../const/socketEvents");
...
const registerChatHandler = (io, socket) => {
    ...
    socket.on(socketEvents_1.ChatEvents.JOIN, async (payload) => {
        const { roomId } = payload || {};
        if (!roomId) {
            socket.emit(socketEvents_1.ChatEvents.ERROR, { message: error_message_1.HttpResponse.CHAT_ID_Required });
            return;
        }
        const chat = await chatService.findChat(roomId);
        if (!chat) {
            socket.emit(socketEvents_1.ChatEvents.ERROR, { message: error_message_1.HttpResponse.CHAT_NOT_FOUND });
            return;
        }
        if (!chat.users.map((id) => id.toString()).includes(userId)) {
            socket.emit(socketEvents_1.ChatEvents.ERROR, { message: error_message_1.HttpResponse.NOT_PERMINTED });
            return;
        }
        socket.join(`chat:${roomId}`);
        await new Promise((r) => setTimeout(r, 100));
        socket.to(`chat:${roomId}`).emit(socketEvents_1.ChatEvents.NOTIFICATION, { roomId });
    });
    socket.on(socketEvents_1.ChatEvents.SEND, async (payload, ack) => {
        try {
            ...
            io.to(`chat:${roomId}`).emit(socketEvents_1.ChatEvents.NEW_MESSAGE, messageToEmit);
    ...
    socket.on(socketEvents_1.ChatEvents.DELIVERED, async ({ roomId, messageId }) => {
        const chat = await chatService.findChat(roomId);
        if (!chat || !chat.users.map(String).includes(userId))
            return;
        await chatService.updateMessage(messageId, { status: "delivered" });
        io.to(`chat:${roomId}`).emit(socketEvents_1.ChatEvents.STATUS_UPDARE, {
            messageId,
            status: "delivered",
        });
    });
    socket.on(socketEvents_1.ChatEvents.READ, async ({ roomId, messageIds }) => {
        await chatService.readMessages(messageIds);
        io.to(`chat:${roomId}`).emit(socketEvents_1.ChatEvents.DELIVERED, {
            messageIds,
            status: "read",
        });
    });
