...
export const NotificationService={
    getNotification:async(userId:string):Promise<INotificationDTO[]>=>{
        try {
            const response=await axiosInstance.get(API.NOTIFICATION.GET_NOTIFICATION(userId))
    ...
    readNotification:async(notifyId:string):Promise<string>=>{
        try {
            const response=await axiosInstance.put(API.NOTIFICATION.READ_NOTIFICATION(notifyId))
