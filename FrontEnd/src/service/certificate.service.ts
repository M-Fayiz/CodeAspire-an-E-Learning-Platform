...
const CertificateService={
    generateCertificate:async(courseId:string,learnerId:string,prgrmTitle:string)=>{
        try {
    ...
    ListCertificate:async(learnerId:string):Promise<ICertificateDTO[]>=>{
        try {
            const response=await axiosInstance.get(API.CERTIFICATE.list_Certificate(learnerId))
            for(let certificate of response.data.certificate as ICertificateDTO[]){
                let pdfURL=await sharedService.getPreSignedDownloadURL(certificate.certificateUrl)
                let prvImage
                if(certificate.preview_image){

                    prvImage=await sharedService.getPreSignedDownloadURL(certificate.preview_image)
                }
                if(pdfURL){
