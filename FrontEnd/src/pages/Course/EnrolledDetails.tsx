...
import type { ICourseProgess, IEnrolledCoursedetailsDTO } from "@/types/DTOS/enrollements.dto";
...
const EnrolledCourseDetails = () => {
  ...
  const [videoUrl, setVideoUrl] = useState({ url: "", title: "" ,lectureId:'',sessionId:''});


  const handleVideoEnd =async (lectureId:string,sessionId:string) => {

    if(lectureId&&sessionId){
      const result=await EnrolledService.updateProgress(enrolledCourse?._id as string,sessionId,lectureId)
      if(result){
        setEnrolledCourse(prv=>
          prv?{...prv,progress:result}:prv
        )
      }
    }
  };
  ...
  const setVideo = (url: string, title: string,lectureId:string,sessionId:string) => {
    setVideoUrl((prev) => ({ ...prev, url, title,lectureId ,sessionId}));
  };
  return (
    <>
      <div className="col-span-3">
          ...
          <div className="relative max-w-5xl mx-auto bg-white  rounded-2xl overflow-hidden border border-gray-200">
            <div className="bg-gradient-to-r from-blue-100 via-blue-50 to-purple-50 p-4">
              <video
                ...
                onEnded={()=>handleVideoEnd(videoUrl.lectureId,videoUrl.sessionId)}
              >
        ...
        <CurriculumProgress progress={enrolledCourse?.progress as ICourseProgess}  />
      </div>
    </>
  );
};
