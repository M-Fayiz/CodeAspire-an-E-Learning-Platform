...
export default function ChatBot({ learnerId, courseId }: ChatbotProps) {
  ...
  const handleSend = async (text = input) => {
    ...
    try {
      const updatedChat = await ChatbotService.createChat(
        learnerId,
        courseId,
        text
      );
  ...
  const markdownComponents: Components = {
    ...
    p({ children }) {
      return <p className="leading-6 mb-2">{children}</p>;
    },
    ...
    <div className="fixed bottom-4 right-4 z-50 sm:bottom-10 sm:right-10">
      {!isOpen && (
        <button
          ...
          rounded-full p-4 shadow-xl transition-all hover:scale-110"
        >
      ...
      {isOpen && (
        ...
        >
          {/* Header */}
          <div className="bg-gradient-to-r from-gray-900 to-gray-800 text-white px-5 py-4 flex justify-between">
          ...
          <div className="flex-1 overflow-y-auto px-4 py-5 bg-gray-50 space-y-6">
            {messages.map((msg, idx) => (
              ...
              >
                <div
                  className={`sm:max-w-[90%] rounded-2xl px-4 py-3 shadow-sm
                    ${
                      msg.role === "user"
