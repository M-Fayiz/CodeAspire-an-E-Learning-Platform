...
export const PasswordChangeForm :React.FC<passWordsProps>= ({userId,setTabs}) => {
  ...
  const handleValidation=()=>{
      ...
      if(passwordData.currentPassword.trim()==''){
        ...
        errors=validatePassword(passwordData.password,passwordData.confirmPassword)
      }
      return errors
  ...
  const handleSubmit = async(e: React.FormEvent) => {
   ...
   if (Object.keys(validateError).length > 0) {
    ...
    try {
       setIsLoading(true)
       const result=await UserService.changePassword(userId,passwordData.currentPassword,passwordData.password)
       if(result){
